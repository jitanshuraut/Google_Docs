...
function Docs() {
  ...
  const save = async () => {
    await addDoc(ref, { name: name, text: content, time: date, email: `${sessionStorage.getItem("email")}` })
    navigate("/docs")
  }
    ...
    <>
      ...
      <div className={style.nav} >
        <div className={style.head_box}>
          <Link to="/docs" style={{ "textDecoration": "none", "color": "black", "display": "flex", "alignItems": "center", "width": "70%" }}>
            ...
            <h1 className={style.heading}>Google Docs /</h1></Link>
        ...
        <div className={style.ext_img_}>
          ...
          <i class="fa-sharp fa-solid fa-trash fa-xl" id={style.ic} onClick={openModal} ></i>
          <Link to="/profile">
            <img src={sessionStorage.getItem("url")} alt="" className={style.profile} />
          </Link>
        </div>
      </div>
      ...
      >
        ...
        <div className={style.delet}  >
          <h1 className={style.d_text}>Do you want to delet this Document </h1>
          <div className={style.d_button}>
            <h3 className={style.yes} onClick={() => { delet(location.state.id) }}>Yes</h3>
            <h3 className={style.no} onClick={() => {
              setno("none")
              setIsOpen(false)
            }}>No</h3>
          </div>
